...
int DATLoader::Open(char *base_path, char *zone_name, Archive *archive) {
	...
	else
	{
		...
		while(ZonPosition < ZonBufferLength)
		{
			...
			{
				Token = GetToken(ZonBuffer, ZonPosition);
				UnitsPerVert = (float)atof(Token.c_str());
#ifdef DEBUGDAT
			...
			{
				Token = GetToken(ZonBuffer, ZonPosition);
				MinExtentX = (float)atof(Token.c_str());
				Token = GetToken(ZonBuffer, ZonPosition);
				MinExtentY = (float)atof(Token.c_str());
				Token = GetToken(ZonBuffer, ZonPosition);
				MinExtentZ = (float)atof(Token.c_str());
#ifdef DEBUGDAT
			...
			{
				Token = GetToken(ZonBuffer, ZonPosition);
				MaxExtentX = (float)atof(Token.c_str());
				Token = GetToken(ZonBuffer, ZonPosition);
				MaxExtentY = (float)atof(Token.c_str());
				Token = GetToken(ZonBuffer, ZonPosition);
				MaxExtentZ = (float)atof(Token.c_str());
#ifdef DEBUGDAT
		...
		for(int j = 0; j < Count4; ++j)
		{
			...
			else
			{
				...
				while(TogPosition < TogBufferLength)
				{
					...
					if(Token == "*POSITION")
					{
						p.x = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
						p.y = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
						p.z = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
					}
					if(Token == "*ROTATION")
					{
						p.rx = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
						p.ry = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
						p.rz = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
					}
					if(Token == "*SCALE")
					{
						p.scale[0] = (float)atof(GetToken(TogBuffer, TogPosition).c_str());
