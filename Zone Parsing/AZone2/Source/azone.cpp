...
QTBuilder::QTBuilder(const char *base_path) {
	_root = NULL;
...
bool QTBuilder::build(const char *shortname,char* &message) {
	...
	message = "Reading zone files...";

	bool V4Zone = false;

	archive = new PFSLoader();

	sprintf(bufs, "%s\\%s.eqg", base_path, shortname);
	if(fff = fopen(bufs, "rb"))
		FileType = EQG;
	else {
		sprintf(bufs, "%s\\%s.s3d", base_path, shortname);
		if(fff = fopen(bufs, "rb")) 
			FileType = S3D;
	}

	if(FileType == UNKNOWN) {
		printf("Unable to locate %s.s3d or %s.eqg: %s\n", shortname, shortname, bufs);
		return(false);
	}
	...
	switch(FileType) {
		case EQG:
			...
			if(fileloader->Open((char *) base_path, (char *) shortname, archive) == 0) {
				delete fileloader;
	...
	message = "Building triangle data...";
	...
	message = "Adding placeable objects...";
...
// Shadows91 -- 2/28/2010
bool QTBuilder::writeOBJ(const char *file) {
	if(_root == NULL)
		return false;
	...
	// vertices
	for(int x = 0; x <  (int)faceCount; x++)
	{
		fprintf(out,"v %f %f %f\n",faceBlock[x].a.x,faceBlock[x].a.y,faceBlock[x].a.z);
		fprintf(out,"v %f %f %f\n",faceBlock[x].b.x,faceBlock[x].b.y,faceBlock[x].b.z);
		fprintf(out,"v %f %f %f\n",faceBlock[x].c.x,faceBlock[x].c.y,faceBlock[x].c.z);
	}
	for(int x = 0; x < (int)_root->countFacelists() * 3; x+= 3)
		fprintf(out,"f %d %d %d\n",x + 1,x + 3,x + 2);
	long MapFileSize = ftell(out);
	fclose(out);	
	printf("Done writing OBJ (%3.2fMB).\n", (float)MapFileSize/1048576);
	return true;
}
...
bool QTBuilder::writeMap(const char *file) {
	...
	head.node_count = (unsigned short)(_root->countNodes());
...
printf("Stopping on box (size) (%.2f -> %.2f, %.2f -> %.2f) at depth %d with %d faces.\n", 
	...
	float c1, c2, c3, c4;
	c1 = (float)(node1? node1->faces.size() : 0);
	c2 = (float)(node2? node2->faces.size() : 0);
	c3 = (float)(node3? node3->faces.size() : 0);
	c4 = (float)(node4? node4->faces.size() : 0);
	
#ifdef MIN_QUADRENT_GAIN
	int miss = 0;
	float gain1 = (float)(1.0 - c1 / cc);
	float gain2 = (float)(1.0 - c2 / cc);
	float gain3 = (float)(1.0 - c3 / cc);
	float gain4 = (float)(1.0 - c4 / cc);
...
void QTNode::doSplit() {
	...
	//find midpoints...
	float midx = (float)(minx + (maxx - minx) / 2.0);
	float midy = (float)(miny + (maxy - miny) / 2.0);
...
void QTBuilder::AddPlaceable(FileLoader *fileloader, char *ZoneFileName, bool ListPlaceable) {
	...
	for(int i = 0; i < fileloader->model_data.plac_count; ++i) {
		...
		// Fix for stupid/retarded negative mesh objects in Sony's zone files
                if(fileloader->model_data.placeable[i]->z < -32727) continue;
                ...
                if(strcmp(ZoneFileName,"tutorialb")) // ignore webs blocking paths
                        if(fileloader->model_data.placeable[i]->model==128 || fileloader->model_data.placeable[i]->model==129 ) continue;
		if(ListPlaceable)
		...
		if(!fileloader->model_data.models[fileloader->model_data.placeable[i]->model]->IncludeInMap && IniFile)
			continue;
		...
		RotX = (float)(fileloader->model_data.placeable[i]->rx * 3.14159 / 180);  // Convert from degrees to radians
		RotY = (float)(fileloader->model_data.placeable[i]->ry * 3.14159 / 180);
		RotZ = (float)(fileloader->model_data.placeable[i]->rz * 3.14159 / 180);
...
void QTBuilder::AddPlaceableV4(FileLoader *fileloader, char *ZoneFileName, bool ListPlaceable) {
	...
	{
		if(!(*Iterator).IncludeInMap && IniFile)
		{
			++OGNum;
			++Iterator;
			continue;
		...
		while(ModelIterator != (*Iterator).SubObjects.end())
		{
			...
			RotX = (float)(fileloader->model_data.placeable[SubModel]->rx * 3.14159 / 180);  // Convert from degrees to radians
			RotY = (float)(fileloader->model_data.placeable[SubModel]->ry * 3.14159 / 180);
			RotZ = (float)(fileloader->model_data.placeable[SubModel]->rz * 3.14159 / 180);
			...
			for(int j = 0; j < model->poly_count; ++j) {
				...
				//
				RotateVertex(v1, (float)((*Iterator).RotX * 3.14159 / 180.0f), 0, 0);
				RotateVertex(v2, (float)((*Iterator).RotX * 3.14159 / 180.0f), 0, 0);
				RotateVertex(v3, (float)((*Iterator).RotX * 3.14159 / 180.0f), 0, 0);

				RotateVertex(v1, 0, (float)((*Iterator).RotY * 3.14159 / 180.0f), 0);
				RotateVertex(v2, 0, (float)((*Iterator).RotY * 3.14159 / 180.0f), 0);
				RotateVertex(v3, 0, (float)((*Iterator).RotY * 3.14159 / 180.0f), 0);
				...
				RotateVertex(Correction, (float)((*Iterator).RotX * 3.14159 / 180.0f), 0.0f, 0.0f);
				...
				//
				RotateVertex(v1, 0.0f, 0.0f, (float)((*Iterator).RotZ * 3.14159 / 180.0f));
				RotateVertex(v2, 0.0f, 0.0f, (float)((*Iterator).RotZ * 3.14159 / 180.0f));
				RotateVertex(v3, 0.0f, 0.0f, (float)((*Iterator).RotZ * 3.14159 / 180.0f));
