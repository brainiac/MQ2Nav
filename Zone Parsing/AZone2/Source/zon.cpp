...
int ZonLoader::Open(char *base_path, char *zone_name, Archive *archive) {
	...
	if(!archive->GetFile(filename, &buffer, &buf_len)) {
		...
		sprintf(fullPath,"%s\\%s",base_path,FSZonName);
		//sprintf(FSZonName,"%s\\%s",base_path,FSZonName);



		printf(".ZON file not found inside EQG. Looking for %s in filesystem. ",fullPath);
		FILE *FSZon = fopen(fullPath, "rb");
	...
	*/
	...
	for(i = 0; i < this->model_data.model_count; ++i) {
		...
		for(j = 0; j < this->model_data.models[i]->tex_count; ++j) {
			...
			for(k = 0; k < tex_count; ++k) {
				...
				// FIX THIS SHIT, it's uninitialized - for now, I have it initialized to NULL at dec, and check if (tex) -- need to investigate what it REALLY should be doing.  - htw
				if(tex) {
	...
	if (this->model_data.zone_model && this->model_data.zone_model->tex)
		this->model_data.zone_model->tex = 0;
	if (this->model_data.zone_model)
