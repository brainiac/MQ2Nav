...
void DoLog(char *Buffer)
{
	try {
		...
		sprintf(Filename,"C:\\NavMesh.log");
		...
		sprintf(szBuffer, "[%s] %s",tmpbuf,Buffer);
		fprintf(fOut,"%s\n", szBuffer);
		fclose(fOut);
	}
	catch(...) {
...
dtTileRef dtNavMesh::addTile(unsigned char* data, int dataSize, int flags, dtTileRef lastRef)
{
	...
	dtMeshHeader* header = (dtMeshHeader*)data;
	if (header->magic != DT_NAVMESH_MAGIC) {
		debug_log("addTile header.magic bad!");
		return 0;
	}
	if (header->version != DT_NAVMESH_VERSION) {
		debug_log("addTile header.version bad!");
		return 0;
	}
		
	// Make sure the location is free.
	if (getTileAt(header->x, header->y)) {
		debug_log("addTile location is not free!");
		return 0;
	}
	...
	else
	{
		...
		if (tileIndex >= m_maxTiles) {
			debug_log("addTile tileIndex bad!");
			return 0;
		}
		...
		if (tile != target) {
			debug_log("addTile could not find the correct location!");
			return 0;
		}
	...
	if (!tile) {
		debug_log("addTile failed tile allocation!");
		return 0;
	}
