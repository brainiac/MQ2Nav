...
public:
	...
	inline const glm::vec3& GetCollideVert(uint32_t index) const
	{
		return collide_verts[index];
	}
	const glm::vec3& GetNonCollideVert(uint32_t index) const
	{
		return non_collide_verts[index];
	}

	const std::vector<uint32_t>& GetCollideIndices() const
	{
		return collide_indices;
	}
	const std::vector<uint32_t>& GetNonCollideIndices() const
	{
		return non_collide_indices;
	}

	std::shared_ptr<EQEmu::EQG::Terrain> GetTerrain() const
	{
		return terrain;
	}
