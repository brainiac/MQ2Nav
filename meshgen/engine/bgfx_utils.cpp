...
{
	utils::setCurrentDir("");
	utils::s_fileReader = BX_NEW(utils::g_allocator, utils::FileReader);
	utils::s_fileWriter = BX_NEW(utils::g_allocator, utils::FileWriter);
}

void utilsShutdown()
{
	bx::deleteObject(utils::g_allocator, utils::s_fileReader);
	utils::s_fileReader = NULL;

	bx::deleteObject(utils::g_allocator, utils::s_fileWriter);
	utils::s_fileWriter = NULL;
}
...
{
	return load(utils::getFileReader(), utils::getAllocator(), _filePath, _size);
}

void unload(void* _ptr)
{
	bx::free(utils::getAllocator(), _ptr);
}
...
{
	return loadShader(utils::getFileReader(), _name);
}
...
{
	return loadProgram(utils::getFileReader(), _vsName, _fsName);
}
...
bgfx::TextureHandle loadTexture(bx::FileReaderI* _reader, const char* _filePath, uint64_t _flags, uint8_t _skip, bgfx::TextureInfo* _info, bimg::Orientation::Enum* _orientation)
{
	...
	uint32_t size;
	void* data = load(_reader, utils::getAllocator(), _filePath, &size);
	if (NULL != data)
	{
		bimg::ImageContainer* imageContainer = bimg::imageParse(utils::getAllocator(), data, size);
...
{
	return loadTexture(utils::getFileReader(), _name, _flags, _skip, _info, _orientation);
}

bimg::ImageContainer* imageLoad(const char* _filePath, bgfx::TextureFormat::Enum _dstFormat)
{
	uint32_t size = 0;
	void* data = loadMem(utils::getFileReader(), utils::getAllocator(), _filePath, &size);

	return bimg::imageParse(utils::getAllocator(), data, size, bimg::TextureFormat::Enum(_dstFormat) );
}
...
void Mesh::load(bx::ReaderSeekerI* _reader, bool _ramcopy)
{
	...
	bx::AllocatorI* allocator = utils::getAllocator();
...
{
	bx::FileReaderI* reader = utils::getFileReader();
...
{
	MeshState* state = (MeshState*)bx::alloc(utils::getAllocator(), sizeof(MeshState) );
	return state;
}

void meshStateDestroy(MeshState* _meshState)
{
	bx::free(utils::getAllocator(), _meshState);
}
